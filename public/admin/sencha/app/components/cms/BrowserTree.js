...
Ext.define('TextoCMS.components.cms.BrowserTree' ,{
	...
	initComponent: function() {
		/*
		if(!Ext.getStore('BrowserTreeStore')){
		
		this.store= Ext.create('TextoCMS.store.medias.BrowserTreeStore');
		}
		else this.store=Ext.getStore('BrowserTreeStore');
		*/
		this.store= Ext.create('TextoCMS.store.medias.BrowserTreeStore');
		
		this.store.model.getProxy().setExtraParam('API', 'Application_Model_FileManager');
		this.store.model.getProxy().setExtraParam('APICall', 'getFolderList');
	    ...
	    this.store.on('load', Ext.bind(this.afterLoad, this));
