Ext.define('TextoCMS.classes.Utils',{
	...
	singleton: true,
	 formatDate:function(v){
	   ...
	   calculateRatio:function(w,h,initialW, initialH){
		 
			if(initialW>initialH){
				ratio=w>initialW?1:w/initialW;
	if(w>initialW)w=initialW;
				h=initialH*ratio;
			}
			else{
				ratio = h>initialH?1:h/initialH;
				if(h>initialH)h=initialH;
				w=initialW*ratio;
			}
