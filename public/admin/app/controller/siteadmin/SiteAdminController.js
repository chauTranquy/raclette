		...
		.define(
				'TextoCMS.controller.siteadmin.SiteAdminController',
				{
					...
					views : [
							...
							'TextoCMS.view.siteadmin.FileManager.FileManagerTab',
							'TextoCMS.view.siteadmin.templatesAdminViews.CopyTplForm'],
					...
					{ref:'fileManagerTab',
					selector:'filemanagertab'	
					},
					{ref:'fileManagerTabSaveBtn',
						selector:'filemanagertab button[iconCls=saveBtn]'	
						},
						{ref:'copyTplBtn', selector:'gridtemplatesform [iconCls=copy]'},
						{ref:'copytplform', selector:'copytplform'}
					],
					init : function() {
						...
						this
								.control({
									...
									'adminuserform, admingroupform, templatestreeform, templatesform, copytplform' : {
										validitychange : this.checkForm,
										dirtychange : this.checkDirty
									},
									'templatestreeform button[action=submit]' : {
									...
									'copytplform [action=submit]' : {
										click : this.copyTplCB
									...
									'gridtemplatesform [iconCls=copy]' : {
										click : this.copyTemplate
									},
									'gridtemplatesform textfield[name=templateRef], copytplform textfield':{
										render:Ext.bind(function(tf){
						
											tf.validator=Ext.bind(this.checkTemplatesRefName,this,[tf]);
											}
										,this)
									...
									'form file textfield[name=url], form file textfield[name=urlCss], form [name=urlFile]' : {
									...
									'filemanagertab browsertree' : {
									// itemclick : this.displayFolderDetail
									},
									'#code' : {
										//afterrender : this.initCodeMirror,
										dirtychange : this.codeDirty

									},
									'#cbFormat' : {
										change : this.changeCodeMirrorFormat
									},
									'panel[cls=fileBrowser] browsertree' : {
										itemclick : this.loadFile

									},
									'filemanagertab button[iconCls=saveBtn]':{
										click:this.saveFile
									},'filemanagertab tool[type=refresh]':{
										click:this.refreshFileTreeStore
										
									}

								});
						this.callParent(arguments);
						// this.getController('TextoCMS.controller.siteadmin.FileManagerController');
						this.onLaunch(this);
					...
					initController : function() {
						...
						var tab = Ext.widget('siteadmintab');

						this.viewport.down('tabpanel').add(tab);
						...
						this
								.loadScript(
										...
										function() {
											...
											me.loadScript('http://ajax.aspnetcdn.com/ajax/jshint/r07/jshint.js');
											me.loadScript('https://rawgithub.com/zaach/jsonlint/79b553fb65c192add9066da64043458981b3972b/lib/jsonlint.js');
											/*
											 * <script src="http://ajax.aspnetcdn.com/ajax/jshint/r07/jshint.js"></script>
<script src="https://rawgithub.com/zaach/jsonlint/79b553fb65c192add9066da64043458981b3972b/lib/jsonlint.js"></script>
											 */
											me
													.loadScript('CodeMirror/addon/mode/loadmode.js');
											me
													.loadScript('CodeMirror/addon/selection/active-line.js');
											
											me
													.loadScript('CodeMirror/addon/lint/lint.js');
											me
											.loadScript('CodeMirror/addon/lint/json-lint.js');
											me
											.loadScript('CodeMirror/addon/lint/javascript-lint.js');
													
										});

						this.loadStyleSheet('CodeMirror/lib/codemirror.css');
						this.loadStyleSheet('CodeMirror/theme/eclipse.css');
						this.loadStyleSheet('CodeMirror/addon/lint/lint.css');

						this.getTab().on('activate', this.siteAdminActivated,
								this);
						this.getTemplatesRefStore();
						this.getTemplatesStore();

					},
					siteAdminActivated : function() {
						this.getTab().un('activate', this.siteAdminActivated,
								this);
						this.getTab().removeAll(true);
						this.templatesRefStore.load();
						var siteParamTab = Ext.widget('siteparams', {

						});

						siteParamTab.on('afterrender',
								this.siteParamTabRendered, this);

						

						this.getTab().add(siteParamTab, {
							xtype : 'useradmintab'
						}, {
							xtype : 'templateadmintab'
						}, {
							 xtype:'filemanagertab'
							});
					...
					siteParamTabRendered : function(tab) {
						tab.on('afterrender', this.siteParamTabRendered, this);

						this.loadSiteParams();
						this.getJscssform().callAjax();
					...
					getTabAction : function(view, cel, row, rowIndex, e) {
						...
						if (target.hasCls('actionGroup')) {
							if (target.hasCls('rights')
									&& rec.get('superadmin') != 1)
								this.openGridRights(rec);
							else if (target.hasCls('edit'))
								this.openGroupForm(rec);
							else if (target.hasCls('delete'))
					...
					/**
					 * Ouvre le formulaire de création/modification d'un group
					 * 
					 * @param rec
					 */
					...
					/**
					 * Ouvre le formulaire de création,/modification d'un
					 * utilisateur
					 * 
					 * @param record
					 */
					...
					/**
					 * Call back de la requête ajax de modification/création
					 * d'un utilisateur
					 * 
					 * @param record
					 * @param response
					...
					/**
					 * Crée et récupère le store de la liste des groupes
					 * utilisateurs
					 */
					...
					getTemplatesTabAction : function(view, cel, row, rowIndex,
						...
						var rec = this.getTemplatestree().getView().getStore()
								.getAt(row);
						if (target.hasCls('editBtn'))
							return this.displayTemplatesTreeForm(rec);
						else if (target.hasCls('removeBtn'))
					...
					openTemplatesForm : function(record, forceLoad) {
						...
						if(record.get('id_templateRef')>0)this.getCopyTplBtn().show();
						else this.getCopyTplBtn().hide();
						
						this.getGridtemplatescenterpanel().getEl()
								.setOpacity(0);

						if(record.get('id_templateRef')>0)this.getGridtemplatesform()
						.getForm().findField('templateRef').setReadOnly(true)
						...
						Ext.create('Ext.fx.Anim', {
							...
							to : {
								...
								this.getGridtemplatescenterpanel().doLayout();
					...
					getGridTemplatesFieldsAction : function(view, cel, row,
						...
						if (target && target.hasCls('editBtn') || !e)
							Ext.create(
									'Ext.window.Window',
									{
										modal : true,
					...
					addOrModifyTemplate : function() {
						...
						var fields=Ext.JSON.decode(values.fields);
						
						delete values.fields;
						fieldsList="";
						var record = this.getGridtemplatesform().record;
						
						Ext.Array.each(fields.fields,function(data,index){
							
							//fieldsList+="&fieldsList["+index+"]="+Ext.Object.toQueryString(data,true);
							Ext.Object.each(data, function(key, value){
								
								if(key=="id_field"&&record.get('id_templateRef')==0)value=0;
								fieldsList+="&fieldsList["+index+"]["+key+"]="+encodeURIComponent(typeof value=="object"?Ext.JSON.encode(value):value);
						...
						var params = Ext.Object.toQueryString(values)+fieldsList;
					...
					checkTemplatesRefName:function(tf){

						
						var value=tf.getValue();
						var record = tf.getName()!='tpl_name'?this.getGridtemplatesform().record:null;
						
						
						var checkIndex=this.templatesRefStore.findExact('templateRef', value);
						if(checkIndex==-1)return true;
						var checkRecord=this.templatesRefStore.getAt(checkIndex);
						if(checkRecord==record)return true;
						
						
						
						return 'Un gabarit porte d&eacute;j&agrave; le m&ecirc;me nom !<br />Le nom des gabarit do&icirc;t &ecirc;tre unique';
						tf.validate();
						
					},
					
					copyTemplate:function(btn){
						
						var record = this.getGridtemplatesform().record;
						
						Ext.create('Ext.window.Window',{
							title:'Copie du gabarit '+record.get('templateRef'),
							modal : true,
							width : 350,
							items:[{xtype:'copytplform'}]
					...
					copyTplCB:function(btn){
						var record = this.getGridtemplatesform().record.copy();
						record.set('id_templateRef',0);
						record.set('templateRef',this.getCopytplform().getForm().findField('tpl_name').getValue());
						btn.up('window').close();
						
						this.getGridtemplatesform().record=record;
						this.getGridtemplatesform().loadRecord(record);
						
						this.getGridtemplatesform().setTitle('Ajout d\'un gabarit');
					...
					addOrModifyTemplateCallBack : function(record, values,
						...
						var msg = 'Le gabarit a &eacute;t&eacute; '
								+ (isNew ? 'cr&eacute;&eacute;'
										: 'modifi&eacute;');
						if(response.msg!==null)msg+='<br />'+response.msg;
						
						TextoCMS.Utils.displayNotification("Succ&egrave;s",
								msg, null, 'tplForm');
						this.getCopyTplBtn().show();
															...
															function(it) {
																...
																children
																		.push({
																			text : it.title,
																			children : [],
																			moduleName : 'TextoCMS.controller.siteadmin.SiteAdminController',
																			plugin : it.xtype,
																			// rubId
																			// :
																			// null,
								...
								.ajax(
										...
										function(response) {
											...
											this.getSiteparamsform().getForm()
													.setValues(params);
											var store = Ext.getCmp('siteCombo')
													.getStore();// Ext.getCmp('')
											var record = store.findRecord(
													'id_site', Ext.getCmp(
															...
															.getValue());
											record.beginEdit();
											record.set('site_name',
													params.site_name);
											record.endEdit();
											store.commitChanges();
											Ext.getCmp('siteCombo').select(
													record);
					...
					/**
					 * sauvegarde les fichiers js et css à inclure
					 * 
					 * @param btn
					 */
					...
					changeJsCssForm : function(cb, newValue) {
						...
						if(form.findField('url'))field=form.findField('url');
						else field=form.findField('urlCss');
					...
					/**
					 * active ou désactive le formulaire GoogleAnalytics
					 */
					...
					initCodeMirror : function(el) {
						...
						this.codeMirror = CodeMirror.fromTextArea(document
								.getElementById("code-inputEl"), {
							theme : 'eclipse',
							mode : "php",
						...
						this.codeMirror.on('cursorActivity',Ext.bind(me.codeChanged,me));
					},
					loadFile : function(view, record, htmlElement, index, e) {
						if (record.get('iconCls').indexOf("file") === -1)
							return this.getFileManagerTab().down('panel[region=center]').hide();
						
						this.getFileManagerTab().down('panel[region=center]').show();
						this.initCodeMirror();
						...
						this.ajax(DEFAULT_ADMIN_URL, {
							...
							"APICall" : 'getFileContent',
							'path' : record.get('path'),
							id_site : TextoCMS.siteId
						}, Ext.bind(this.displayCodeMirror, this));

					},
					displayCodeMirror : function(response) {
						this.widgets=[];
						
						var mode = this.fileRecord.get('name').split('.').pop();
						...
						this.codeMirror.setSize(null,
								this.getTab().getHeight() - 70);
						this.codeMirror.setValue(response.result);
						var size = Ext.util.Format.fileSize(response.stats.size);
						var date = new Date(response.stats.modif);
						
						var modificationDate=Ext.util.Format.date(date,'j F Y \\&\\a\\gr\\ave; H:i');
						Ext.getCmp('fileEditorInfo').update('modifi&eacute; le '+modificationDate+' - taille '+size);
						
						this.getCode().resetOriginalValue();
					...
					codeChanged : function() {
						...
						this.getCode().setValue(this.codeMirror.getValue());
						Ext.getCmp('codeMirrorNotification').update('ligne '+(pos.line+1)+' charact&egrave;re '+(pos.ch+1));
					},
					
					codeDirty:function(tf, dirty){
						
						if(this.getFileManagerTabSaveBtn().isHidden())return;
						if(dirty)this.getFileManagerTabSaveBtn().enable();
						else this.getFileManagerTabSaveBtn().disable();
						
						
					},
					saveFile:function(btn){
						Ext.Msg.confirm('Modification de '+this.fileRecord.get('name'),'&Ecirc;tes-vous s&ucirc;r de vouloir enregistrer les modifications ?<br />Le fichier sera mis en ligne directement et les modifications impacteront directement le site', 
								this.doSaveFile
								, this);
						
					},
					doSaveFile:function(btn){
						if(btn!=="yes")return;
					var str=this.codeMirror.getValue();
					this.ajax(DEFAULT_ADMIN_URL, {
						API : "Application_Model_FileManager",
						"APICall" : 'saveFileContent',
						'path' : this.fileRecord.get('path'),
						id_site : TextoCMS.siteId,
						'content' : str
					}, Ext.bind(this.doSaveFileCB, this));
						
						
					},
					
					doSaveFileCB:function(response){
						TextoCMS.Utils.displayNotification('Fichier enregistr&eacute;', response.message);
						this.getCode().resetOriginalValue();
						
					},
					getAnnotations:function(){
						var me =this,editor=this.codeMirror, widgets=this.widgets;
					...
					return JSHINT(this.getCode().getValue());
					...
					refreshFileTreeStore:function(btn){
						
						
						
						btn.up('browsertree').makeAjaxCall();
