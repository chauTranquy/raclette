Ext
		.define(
				'TextoCMS.controller.medias.MediasController',
				{
					...
					refs : [ {
						...
						ref : 'deleteFolderBtn',
						selector : 'mediastab browsertoolbar [iconCls=deleteFolder]'
					}, {
						ref : 'deleteFileBtn',
						selector : 'mediastab browsertoolbar [iconCls=deleteFile]'
					}, {
						ref : 'addFileBtn',
						selector : 'mediastab browsertoolbar [iconCls=addFile]'
					},{
						ref : 'modifyFileBtn',
						selector : 'mediastab browsertoolbar [iconCls=modifyFile]'
					}, {
						ref : 'folderForm',
					...
					init : function() {
						...
						this.control({
							'#mediasBrowserTree' : {
								...
								beforeitemexpand:this.beforeFolderExpanded
								/*,
							...
							'mediastab browsertoolbar trigger' : {
								change : this.filterResult

							},
							'mediastab browsertoolbar [iconCls=addFolder]' : {
								click : this.openAddFolderForm

							},
							'mediastab browsertoolbar [iconCls=deleteFolder]' : {
								click : this.deleteFolder

							},
							'mediastab browsertoolbar [iconCls=deleteFile]' : {
								click : this.deleteFile

							},
							'mediastab browsertoolbar [iconCls=addFile]' : {
						...
						this.getController('TextoCMS.controller.medias.ImageEditorController');
						this.onLaunch(this);
					...
					initController:function(){
						...
						this.viewport.down('[region=center]').add(Ext.widget('mediastab'));
						
								this.mediasTree = Ext
								.create('TextoCMS.components.cms.BrowserTree',{id:'mediasBrowserTree', recursive:true});
					...
					displayFolderDetail : function(view, record, htmlElement,
						...
						if (!this.getIconBrowser()) {
							...
							var browserPanel = Ext.widget('iconbrowser',{
								xtype : 'iconbrowser',
							...
							browserPanel.store.on('load',this.selectFirstElement, this);
							...
							if(this.getIconBrowser().plugins[0]&&this.getIconBrowser().plugins[0].tracker)this.getIconBrowser().plugins[0].tracker.onEnd=Ext.Function.createDelayed(function(e) {
						        ...
						        Ext.Array.each(selModel.getSelection(), function(rec){
						        	...
						        	if(rec.get('type')=="folderBack")dataview.deselect(rec);
					        ...
					        me.mediaSelectionChanged(me.getIconBrowser(),me.getIconBrowser().getSelectionModel().getSelection());
						...
						//	this.centerPanel.doLayout();
							...
							this.getIconBrowser().getEl().un('keypress', this.keyPressed,this);
							
						};
						
						this.getIconBrowser().getEl().on('keypress', this.keyPressed,this);
						

					},
					selectFirstElement:function(st){
						
						this.currentView.getSelectionModel().select(st.getAt(0));
						this.currentView.getEl().dom.focus();
					...
					keyPressed:function(e){
						...
						case Ext.EventObject.DOWN:
							if(this.currentView==this.getIconBrowser())return e.stopEvent();
							else this.fileInfo(this.getIconBrowser(),this.currentView.getSelectionModel().getSelection()[0]);	
						case Ext.EventObject.LEFT:
						case Ext.EventObject.RIGHT:
							
						if(this.currentView!==this.getIconBrowser())return e.stopEvent();	
							this.fileInfo(this.getIconBrowser(),this.currentView.getSelectionModel().getSelection()[0]);
							break;
						case Ext.EventObject.ENTER:
						if(this.currentView.getSelectionModel().getSelection()[0].get('type')=='file')this.getInfoPanel().getEl().down('a').dom.click();
						else this.navigateBrowser(this.getIconBrowser(),this.currentView.getSelectionModel().getSelection()[0]);
						
						break;
						case Ext.EventObject.DELETE:
							if(this.currentView==this.getIconBrowser())	{
							if(this.getIconBrowser().getSelectionModel().getSelection()[0].get('type')=='file')	this.deleteFile();
							else this.deleteFolder();
							}
							break;
						}
					...
					refreshTree : function(btn) {
						...
						this.getBrowserTree().loadCB=Ext.bind(
								this.getCurrentRecord, this);
						
	
						this.getBrowserTree().baseFolder=this.getBrowserTree().rootparams=this.getBrowserTree().selectedNode=null;

						
						
						btn.up('panel').down('treepanel').makeAjaxCall();

					},
					beforeFolderExpanded:function(node){
						if(!node.hasChildNodes()&&!node.isLeaf()){
							
							this.getBrowserTree().rootparams=node.get('path');
							this.getBrowserTree().selectedNode=node;
							this.getBrowserTree().makeAjaxCall();
						}
					...
					changeView : function(btn) {
						...
						if (tpl) {
							...
							this.getIconBrowser().getEl().dom.focus();
						...
						var selMod=Ext.create('Ext.selection.RowModel',{selectionMode:'MULTI'});
						
						
						
						this.currentView = (!this.getMediadetailGrid()?Ext.widget({
							xtype : 'mediadetailGrid',store:this.getIconBrowser().store, selModel:selMod
						...
						this.currentView.doLayout();
							...
							this.getMediadetailGrid().view.getEl().un('keypress', this.keyPressed,this);
							this.getMediadetailGrid().view.getEl().on('keypress', this.keyPressed,this);
							this.getMediadetailGrid().view.getEl().dom.focus();
					...
					/*	if (record.get('type') != 'file')
						...
						this.getModifyFileBtn().setDisabled(arrayExt['image'].indexOf(record.get('ext').toLowerCase())==-1);
						
						if(record.get('type') != 'folderBack')this.getInfoPanel().show().loadRecord(record);
					...
					addFile : function() {
						...
						var val = this.getUploadForm().getValue().replace(/.*(\/|\\)/, '');
						
						if (this.getUploadForm().findParentByType('uploadform').browser.store.findExact('name', val) > -1) {
							...
							Ext.Msg
									.show({
										...
										fn : function(btn) {
											if (btn == 'ok') {
												this.getUploadForm().findParentByType('form').getForm().baseParams.overwrite = 1;
												this.doUpload();
											}
										}
									});
					 ...
					 */
					doUpload : function() {
						var uploadForm =this.getUploadForm().findParentByType('uploadform'); 
						
						this.getUploadForm().findParentByType('form')
								.getForm()
								.doAction(
										'submit',
										{
					...
					deleteFile : function(btn) {
						...
						selection.each(function(record){
							if(record.get('type')!=='folder'&&record.get('type')!=='folderBack') arrayRecord.push(record);
							else this.currentView.getSelectionModel().deselect(record);
						},this);
						
						if(arrayRecord.length==0) return;
						selection=arrayRecord;
	...
	var arrayRecord=[];
						...
						selection.each(function(record){
							if(record.get('type')!=='folder'&&record.get('type')!=='folderBack') this.currentView.getSelectionModel().deselect(record);
							else arrayRecord.push(record);
						},this);
					...
					getRightsItems : function() {
						...
						Ext.getCmp('siteCombo').getStore().each(function(record){
								...
								children : [],
								moduleName : 'TextoCMS.controller.medias.MediasController',
								//plugin : null,
					...
					doUploadFiles:function(overwrite){
						...
						datas.append('baseFolder', this
								.getUploadForm()
								.down('form').baseParams.baseFolder!==null?this
										.getUploadForm()
						 ...
						 this.xhr.onreadystatechange = function() {
							...
							return;
									...
									.show({
							 		...
							 		if(me.selectedFileIndex+1==me.FileReaderStore.getCount()){
							 		msg='T&eacute;l&eacute;chargement'+(me.FileReaderStore.getCount()>1?'s':'')+' termin&eacute;'+(me.FileReaderStore.getCount()>1?'s':'');
					    			
					    			TextoCMS.Utils.displayNotification("Succ&egrave;s",
											msg, null,null);
					    			return me.getFileReaderDataview().findParentByType('window').close();
							 		}
							 		//me.getUploadForm().store.removeAt(0);
							 		  me.selectedFileIndex++;
									  me.doUploadFiles();
					...
					updateProgress:function(e){
						...
						//  if(currentState>=100){
						    ...
						    if(this.selectedFileIndex+1<this.FileReaderStore.getCount()){
							  
						    msg='T&eacute;l&eacute;chargement'+(this.FileReaderStore.getCount()>1?'s':'')+' en cours '+ (this.selectedFileIndex+1)+' sur '+this.FileReaderStore.getCount();
					...
					mediaSelectionChanged:function(view, selected){
						...
						if(rec.get('type')=="folder"){
							folderCount++;
							this.getDeleteFolderBtn().setDisabled(false);//this.getIconBrowser().deselect(rec);
						}
						...
						this.getDeleteFileBtn().setDisabled(newSelection.length==0||newSelection.length==folderCount);
					...
					initFileReaderDataview:function(){
						...
						if($$('.dragload').length>0){
					
						
							
							if("undefined"!==typeof jQuery){
								jQuery.noConflict();
								//console.log('oui', dragload);
								//return;
								
							}
							
							var dragload=this.getFileReaderDataview().el.dom;//$($$('.dragload')[0]);
					...
					Ext
						...
						return;
							...
							console.log(this.getFileReaderDataview().el);
							dragload.un('dragover',Ext.bind(this.handleDragOver, this));
							dragload.un('drop',Ext.bind(this.handleFileSelect, this));
							
							dragload.on('dragover',Ext.bind(this.handleDragOver, this));
							dragload.on('drop',Ext.bind(this.handleFileSelect, this));
							
							return;
							console.log(dragload);
