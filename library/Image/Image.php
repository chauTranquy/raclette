...
class Image {
    ...
    var $fontPath="";
    var $fontSize="11";
    var $watermarkText="";// &copy; Warner Bros 2010";
    ...
    function Image($name='') {
        ...
        $this->fontPath=dirname(__FILE__).'/timesbd.ttf';
    }
...
function save($show=false, $watermark=false, $resource=false) {
 ...
 imagefilledrectangle($new_image,0,0,$this->width,$this->height,$col);
 imagealphablending($new_image,true);
        ...
        $white=imagecolorallocatealpha($new_image, 255, 255, 255, 50);
        
        $dimText=imagettfbbox($this->fontSize, 0, $this->fontPath, $this->watermarkText);
        
       //print_r($dimText);
        
        $widthText=abs($dimText[2]-$dimText[0]);
        $heightText=abs($dimText[1]-$dimText[5]);
        
        $startX=($this->width-$widthText)/2;
        
        $startY=($this->height-$heightText)+$this->fontSize-3;
        
        //die($startY);
        
        imagefttext($new_image,$this->fontSize,0,$startX,$startY,$white,$this->fontPath,$this->watermarkText);
        endif;
        
        $name = $show ? null: $this->dir.DIRECTORY_SEPARATOR.$this->name.'.'.$this->ext;
        
        //die();
          if($show&&!$resource) @header('Content-Type: image/'.$this->type);
